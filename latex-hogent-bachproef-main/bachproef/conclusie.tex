...
\vspace{1em}Het ontwikkelde ML-model op basis van een Random Forest classifier toont aan dat het haalbaar is om een geautomatiseerd taakallocatiesysteem te bouwen dat meerdere factoren tegelijk afweegt: rol-match, persona-fit, werkdruk en taakaantal. De vergelijkende evaluatie met Round-Robin, Greedy Rol-Match en ChatGPT (GPT-4o) toont aan dat het ML-model de enige methode is die beide 
dimensies van het probleem balanceert: het levert een aanvaardbare toewijzing en een eerlijke spreiding van de werklast over het volledige team. Concreet behaalt het model een werklastverdeling met een standaarddeviatie van 1,37, dat aanzienlijk lager ligt dan Greedy (3,13) en ChatGPT (3,35), terwijl elke werknemer minimaal drie taken ontvangt en niemand meer dan 7.

Methoden die uitsluitend optimaliseren voor rol-match, zoals Greedy en ChatGPT, presteren inhoudelijk sterk maar creëren onevenwichten waarbij bepaalde werknemers worden overbelast en anderen 
volledig worden overgeslagen. Precies dat soort onevenwicht ligt aan de basis van de werkstress die deze proef wil aanpakken. Het model geeft dus een concreet, onderbouwd antwoord op de onderzoeksvraag: AI kan ingezet worden om taakverdeling te optimaliseren, op voorwaarde dat het systeem werkdruk en welzijn als expliciete criteria opneemt in het leerproces.

\vspace{1em}Daarnaast werd ook de deelvraag beantwoord:

\vspace{0.5em}``Hoe creëer je zo een AI-model en welke resources heb je hiervoor nodig?''

Het ontwikkelde systeem toont aan dat de technische drempel voor een werkend prototype lager ligt dan vaak wordt aangenomen. Het model is volledig opgebouwd in Python, met \textbf{scikit-learn} als enige machine learning-bibliotheek. Het draait op standaard hardware zonder nood aan GPU's of cloudinfrastructuur. De architectuur bestaat uit vier module scripts: \textbf{datasetbuilder.py} voor 
de constructie van feature-vectoren, \textbf{trainmodel.py} voor de training van de Random Forest classifier, \textbf{predictor.py} voor het berekenen van geschiktheidsscores en \textbf{taskallocator.py} voor de eigenlijke toewijzingslogica. Invoerdata wordt beheerd via twee JSON-bestanden die de werknemers- en taakprofielen bevatten.
...
\vspace{1em}De meerwaarde van deze bachelorproef ligt in de benadering van het allocatieprobleem. Veel bestaande systemen, zoals het door \textcite{Arslan_2024} beschreven model focussen vrijwel uitsluitend op skill- en rol-matching. Dit onderzoek voegt een extra dimensie toe door ook stressniveau, persona en de dynamische evolutie van werkdruk na elke toewijzing mee te nemen in het model. Dit maakt het een meer duurzaam en mensgericht alternatief voor prestatiegerichte allocatiesystemen.

Bovendien biedt het onderzoek een vergelijkingsbasis tussen vier fundamenteel verschillende allocatiestrategieën, wat de relatieve sterktes en zwaktes van elk model concreet zichtbaar maakt.

\section{Kritische reflectie}
\vspace{1em}Toch zijn er beperkingen die eerlijk benoemd moeten worden. De voornaamste is de afwezigheid van historische data. Omdat het model in essentie leert om een zelfopgesteld heuristisch puntensysteem na te bootsen, kan het geen patronen ontdekken die buiten die vooraf gedefinieerde regels vallen. Het systeem functioneert correct als proof of concept, maar zou voor productiegebruik getraind moeten worden op echte historische toewijzingsdata. Zonder die stap biedt het systeem geen aantoonbare meerwaarde ten opzichte van het onderliggende regelgebaseerde model zelf.
...
Verder roepen de persona-kenmerken ethische vragen op over hoe deze scores in de praktijk bepaald worden. Subjectieve beoordelingen door leidinggevenden kunnen persoonlijke 
vooroordelen inbakken in het model op een manier die moeilijker te detecteren is dan bij een menselijke beslissing. Een robuust systeem vereist objectief meetbare inputdata en transparante beslissingslogica, denkende aan de AVG-vereisten rond uitlegbaarheid van geautomatiseerde beslissingen.

Wat betreft de verwachte uitkomst: de hypothese dat een ML-model beter zou presteren op het vlak van werklastbalans werd bevestigd. Minder verwacht was het zwakke resultaat op rol-match (46\%) in vergelijking met Greedy en ChatGPT (beiden 100\%). Dit is echter geen fout in het model, maar een gevolg van de afweging: het model verkiest soms een werknemer met een iets minder perfecte rol maar een lagere werkdruk, boven een perfecte kandidaat die al overbelast is. Of deze afweging in de praktijk wenselijk is, hangt af van de organisatiecontext. Hier kan wel verder onderzoek gedaan 
worden door het systeem te testen binnen een reële bedrijfsomgevin. Hier geven leidinggevenden en werknemers feedback op de gegenereerde toewijzingen. Op basis van die feedback kan dan bepaald worden welke afweging tussen rol-match en werklastbalans het meest aansluit bij de noden van de organisatie en kunnen de gewichten in het model bijgestuurd worden.

\section{Aanbevelingen voor verder onderzoek}

\vspace{1em}Ten eerste is er nood aan validatie met echte historische toewijzingsdata uit een bedrijfsomgeving, om te toetsen of de geleerde patronen overeenkomen met wat menselijke 
planners als optimaal beschouwen. Ten tweede biedt de integratie van reinforcement learning, waarbij het systeem leert via feedback op werkelijke uitkomsten in plaats van gesimuleerde scores, een veelbelovende richting voor een adaptief systeem dat zichzelf continu verbetert op basis van ervaringen uit de praktijk. Ten derde zou een longitudinale studie kunnen nagaan of een evenwichtigere taakverdeling wel effectief leidt tot lagere stressniveaus en minder burn-out, wat de kern is van de oorspronkelijke probleemstelling.
