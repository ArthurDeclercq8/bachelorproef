...
Op basis van deze data wordt een machine learning-model ontwikkeld met behulp van scikit-learn. Het model is een regressiemodel dat leert een geschiktheidsscore te voorspellen voor elke mogelijke combinatie van werknemer en taak. Deze score is gebaseerd op kenmerken zoals skill-match, werkdruk, stressniveau en taakcomplexiteit. Tijdens de trainingsfase wordt het model gevoed met synthetisch gegenereerde voorbeelden waarin een optimale score wordt berekend op basis van een gewogen combinatie van deze factoren. Hierbij wordt een onderscheid gemaakt tussen trainingsdata en testdata: de trainingsdata verwijst naar de specifieke dataset die gebruikt wordt om het model te fitten. Testdata is een breder begrip dat alle informatie omvat waaruit het model patronen kan afleiden, inclusief eventueel afgeleide kenmerken of gesimuleerde optimalisatiescores.
...
\subsection{Datasets: werknemers en taken}
...
Het tweede bestand, tasks.json, bevat 300 te verdelen taken. Elke taak heeft een uniek identificatienummer, een omschrijving, een vereiste functie, een taaktype (development, support, management of devops), een moeilijkheidsgraad en het aantal resterende dagen tot de deadline.
\begin{figure}
    \centering
...
\vspace{1em}De eerste module, datasetbuilder.py, zet de ruwe data om naar numerieke feature-vectoren die het machine learning-model kan verwerken. De functie: buildrolemap verzamelt alle unieke rollen uit zowel de werknemers- als taakdata, sorteert ze alfabetisch en kent aan elke rol een geheel getal toe. Deze map wordt opgeslagen als rolemap.json. De hulpfunctie: encoderole converteert een rol naar het bijbehorende getal en geeft de waarde -1 terug wanneer de rol onbekend is. De centrale functie: buildfeatures bouwt per combinatie van werknemer en taak een vector van 16 elementen op. De eerste twee elementen zijn de gecodeerde rol van de werknemer en die van de taak. Daarna volgen de ervaring en de werkdruk van de werknemer, gevolgd door de tien persona-kenmerken. De vector sluit af met de moeilijkheidsgraad en het aantal resterende dagen van de taak.
\begin{figure}
    \centering
...
Na de training wordt het model geëvalueerd op de testset via accuracy (hoeveel procent correct), een classificatierapport met precision (van alle voorspelde positieven, hoeveel waren echt positief), recall (van alle echte positieven, hoeveel werden gevonden) en de F1-score (het evenwicht tussen precision en recall in één getal, waarbij een waarde dichter bij 1 beter is). Daarnaast wordt een confusion matrix opgesteld, een tabel die toont hoeveel fouten van welk type er gemaakt werden. Tot slot worden de acht meest bepalende kenmerken gerapporteerd, gemeten via de mate waarin elk kenmerk bijdraagt aan de beslissingen van het model. \begin{figure}
    ...
    \caption[trainmodel.py (derde deel)]{\label{fig:screenshot6}Derde deel van de code van module: trainmodel.py}
\end{figure}

\subsection{De derde module: predictor.py}
\vspace{1em}De derde module: predictor.py, laadt het opgeslagen model en berekent voor een gegeven werknemer-taakcombinatie de kans dat de werknemer de beste keuze is. Daarvoor wordt de methode: predictproba gebruikt, die twee kansen teruggeeft: de kans op klasse 0 (niet de beste keuze) en de kans op klasse 1 (de beste keuze). Enkel die laatste waarde wordt als score van het machine doorgegeven aan de toewijzingslogica.
\begin{figure}
    \centering
